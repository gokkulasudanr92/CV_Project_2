...
def main(unused_argv):
    ...
    train_data, train_labels = extract_data_images_from_folder(FACES_TRAINING_SRC_LOCATION, NON_FACES_TRAINING_SRC_LOCATION, 0, 50)
    normalized_train_data = normalize_data_to_negative_one_to_positive_one(train_data)

    eval_data, eval_labels = extract_data_images_from_folder(FACES_TRAINING_SRC_LOCATION, NON_FACES_TRAINING_SRC_LOCATION, 100, 10)
    normalize_eval_data = normalize_data_to_negative_one_to_positive_one(eval_data)

    # Create CNN Estimator
    classifier = tf.estimator.Estimator(model_fn=cnn_model_fn, model_dir="../temp/final/classifier_cnn_model_SGD_2")
